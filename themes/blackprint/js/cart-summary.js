...
$(document).ready(function(){
	...
	$('.cart_quantity_delete').off('click').on('click', function(e){
		e.preventDefault();
		if ($(this).parent().parent().is('[class^="product_option_for_"]')) {
            deletedQuantity = 1;
		} else {
            deletedQuantity = $(this).parent().parent().find('.cart_quantity span').text();
        }
    ...
    $(document).on('click', '.attribute_toggle', function () {
        $(this).parent().find('.attribute_list_container').slideToggle();
...
(function ($) {
	$.fn.wrapTextLines = function (options) {
		var settings = $.extend({
			'leftClass': 'attribute_title product-cart-option-title',
			'rightClass': 'attribute_value product-cart-option-value product-option-chosen_period'
		}, options);
		...
		return this.each(function () {
			...
			container.html(function (index, lineHtml) {
				return lineHtml
					.replace(/\s@predesire@\s+:/, '')
					.replace(/^([^:]+:)(.+)$/gmi, '<span class="' + settings.leftClass + '">$1</span><span class="' + settings.rightClass + '">$2</span><br />');
			});
		});
	};
})(jQuery);